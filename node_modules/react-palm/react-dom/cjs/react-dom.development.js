...
var didError=true;var returned=null;// Keeps track of the value of window.event so that we can reset it
...
if(typeof window.event!=='undefined'&&window.hasOwnProperty('event')){window.event=windowEvent;}returned=func.apply(context,funcArgs);didError=false;}// Create a global error event handler. We use this to capture the value
...
window.removeEventListener('error',handleWindowError);return returned;};invokeGuardedCallbackImpl=invokeGuardedCallbackDev;}}var invokeGuardedCallbackImpl$1=invokeGuardedCallbackImpl;// Used by Fiber to simulate a try-catch.
 ...
 */function invokeGuardedCallback(name,func,context,a,b,c,d,e,f){hasError=false;caughtError=null;return invokeGuardedCallbackImpl$1.apply(reporter,arguments);}/**
 ...
 * @param {*} context The context to use when calling the function
 * @param {...*} args Arguments for function
 */function invokeGuardedCallbackAndCatchFirstError(name,func,context,a,b,c,d,e,f){var returned=invokeGuardedCallback.apply(this,arguments);if(hasError){var error=clearCaughtError();if(!hasRethrowError){hasRethrowError=true;rethrowError=error;}}return returned;}/**
 ...
 */function executeDispatch(event,listener,inst){var type=event.type||'unknown-event';event.currentTarget=getNodeFromInstance(inst);var returned=invokeGuardedCallbackAndCatchFirstError(type,listener,undefined,event);event.currentTarget=null;return returned;}/**
...
returned.push(executeDispatch(event,dispatchListeners[i],dispatchInstances[i]));}}else if(dispatchListeners){returned.push(executeDispatch(event,dispatchListeners,dispatchInstances));}event._dispatchListeners=null;event._dispatchInstances=null;return returned;}/**
 ...
 * @private
 */var executeDispatchesAndRelease=function executeDispatchesAndRelease(event){if(event){var returned=executeDispatchesInOrder(event);if(!event.isPersistent()){event.constructor.release(event);}return returned;}return[];};var executeDispatchesAndReleaseTopLevel=function executeDispatchesAndReleaseTopLevel(e){return executeDispatchesAndRelease(e);};function isInteractive(tag){return tag==='button'||tag==='input'||tag==='select'||tag==='textarea';}function shouldPreventMouseEvent(name,type,props){switch(name){case'onClick':case'onClickCapture':case'onDoubleClick':case'onDoubleClickCapture':case'onMouseDown':case'onMouseDownCapture':case'onMouseMove':case'onMouseMoveCapture':case'onMouseUp':case'onMouseUpCapture':return!!(props.disabled&&isInteractive(type));default:return false;}}/**
...
var processingEventQueue=eventQueue;eventQueue=null;var handlerReturnValues=[];if(!processingEventQueue){return;}forEachAccumulated(processingEventQueue,function(event){handlerReturnValues=handlerReturnValues.concat(executeDispatchesAndReleaseTopLevel(event));});if(handlerReturnValues.length>0){reportStuffReturnedFromEventHandlers(handlerReturnValues);}!!eventQueue?invariant(false,'processEventQueue(): Additional events were enqueued while processing an event queue. Support for this has not yet been implemented.'):void 0;// This would be a good time to rethrow if any of the event handlers threw.
